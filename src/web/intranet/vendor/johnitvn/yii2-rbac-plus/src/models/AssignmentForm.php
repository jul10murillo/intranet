...
class AssignmentForm extends Model {
    ...
    public function __construct($userId, $config = array()) {
        ...
        $this->userId = $userId;
        $this->authManagerldap = Yii::$app->authManagerldap;
        foreach ($this->authManagerldap->getRolesByUser($userId) as $role) {
    ...
    public function save() {
        $this->authManagerldap->revokeAll(intval($this->userId));
        if ($this->roles != null) {
            foreach ($this->roles as $role) {
                $this->authManagerldap->assign($this->authManagerldap->getRole($role), $this->userId);
            }
        }
        return true;
    }
