...
class LdapController extends Controller {
    ...
    public function actionImportAllUsers() {
        ...
        foreach ($userNamesToAdd as $userName) {
            ...
            $user = \Edvlerblog\Adldap2\model\UserDbLdap::findByUsername($userName);
            if (is_null($user)) {
    ...
    private function removeUser($userName){
        ...
        $model = $connection->createCommand('DELETE FROM user WHERE username=:userid');
        $model->bindParam(':userid', $user);
        $user = $userName;
        $model->execute();
